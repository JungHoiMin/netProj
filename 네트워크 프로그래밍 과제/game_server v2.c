		...
		for (i = 0; i < reads.fd_count; i++) {
			if (FD_ISSET(reads.fd_array[i], &cpyReads)) {
				if (reads.fd_array[i] == hServSock) {     // connection request!
					...
					if (start_flag != 111) {	// 시작 플래그를 받지 못했을 시 받을 때까지 대기
						if (reads.fd_array[i] == reads.fd_array[1]) {
							recv(reads.fd_array[i], &start_flag, 4, 0);
							for (j = 2; j < reads.fd_count; j++) {	// 모든 공격자에게 시작 플래그 전송
								send(reads.fd_array[j], &start_flag, 4, 0);
							}
							printf("Game Start\n");
						}
					}
					...
					if (strLen <= 0) {    // close request!
						...
						if (i == 1) {	// 방어자 클라이언트가 소켓 종료 요청을 하면
							for (j = 0; j < cpyReads.fd_count; j++) {	// 모든 공격자 클라이언트와의 소켓 종료 및 서버 종료
						...
						if (clientInfo == 1) {	// 1: 방어자
							...
							if (dead_client != -1) {	// 파괴된 공격자가 있으면
								printf("closed client: %d, StrLen:%d \n",
									cpyReads.fd_array[dead_client + 1], strLen);
								closesocket(cpyReads.fd_array[dead_client + 1]);	// 파괴된 공격자와의 소켓 연결 종료
						...
						if ((send_buf[0] == 0) || (send_buf[0] == send_buf[(i - 1) * 3])) {
							send(reads.fd_array[1], &send_buf, (send_buf[0] + 1) * sizeof(int) * 3, 0);
						}
					}
				}
			}
